...
def _auto_locate_managers(start_dir: Path) -> dict[str, ManagerConfig]:
    """Auto-locate managers by searching for __manager__ in Python files.
...
def _has_manager(file_path: Path) -> bool:
    """Check if a Python file contains __manager__."""
    try:
        content = file_path.read_text()
        return "__manager__" in content
    except Exception:
        return False
