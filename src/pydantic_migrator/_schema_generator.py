...
class SchemaGeneratorBase(ABC, Generic[SchemaType]):
    ...
    def _get_nested_model_dependencies(self: Self, model: type[BaseModel]) -> set[str]:
        """Get the class names of nested BaseModels referenced by a model's fields.

        Inspects all regular fields and computed fields to find direct references
        to other BaseModel classes.

        Args:
            model: Pydantic model class to inspect.

        Returns:
            Set of class names that this model depends on.
        """
        deps: set[str] = set()
        for field_info in model.model_fields.values():
            self._collect_deps_from_type(field_info.annotation, deps)
        if hasattr(model, "model_computed_fields"):
            for computed_info in model.model_computed_fields.values():
                if hasattr(computed_info, "return_type"):
                    self._collect_deps_from_type(computed_info.return_type, deps)
        return deps

    def _collect_deps_from_type(self: Self, python_type: Any, deps: set[str]) -> None:
        """Recursively collect BaseModel class names referenced by a type annotation.

        Traverses through generic types (List, Dict, Optional, Union, etc.) to find
        all BaseModel references.

        Args:
            python_type: Python type annotation to inspect.
            deps: Mutable set to collect dependency class names into.
        """
        if (
            python_type is None
    ...
    def _topological_sort_nested_models(  # noqa: C901
        ...
        """Sort nested models so that dependencies come before dependents.

        Uses Kahn's algorithm for topological sorting. This ensures that when generating
        schemas with non-hoisted declarations (e.g., Zod ``const``), a model's
        dependencies are defined before it is referenced.

        Args:
            nested_models: Dictionary mapping class names to model classes.
            root_name: The root model's class name, which is excluded from the sorted
                output (it is generated separately as the main schema).

        Returns:
            List of (name, model) tuples in dependency order.
        """
        names_in_nested = set(nested_models.keys()) - {root_name}

        if not names_in_nested:
            return []

        # Build dependency graph: model_name -> set of names it depends on
        deps_of: dict[str, set[str]] = {}
        for name in names_in_nested:
            model = nested_models[name]
            all_deps = self._get_nested_model_dependencies(model)
            deps_of[name] = all_deps & names_in_nested

        # Build reverse graph: model_name -> set of names that depend on it and compute
        # in-degrees (number of dependencies each node has)
        dependents_of: dict[str, set[str]] = {name: set() for name in names_in_nested}
        in_degree: dict[str, int] = {name: len(deps) for name, deps in deps_of.items()}

        for name, deps in deps_of.items():
            for dep in deps:
                dependents_of[dep].add(name)

        # Seed queue with nodes that have no dependencies (in_degree 0)
        ...
        while queue:
            name = queue.popleft()
            result.append((name, nested_models[name]))
        ...
        # Fallback: append anything not yet included (circular references)
        for name in sorted(names_in_nested):
