...
class TypeScriptSchemaGenerator(SchemaGeneratorBase[TypeScriptModule]):
    ...
    def generate_schema(
        ...
        auxiliary: list[str] = []
        sorted_nested = self._topological_sort_nested_models(
            self._nested_models, model.__name__
        )
        for nested_name, nested_model in sorted_nested:
            nested_schema = self._generate_schema_for_model(nested_model, nested_name)
